...
leffIdent =
  styleReserved .~ HSet.fromList ["let", "in", "val", "perform", "inst", "with", "handle", "handler", "fun"] $
    emptyIdents
...
app =
  foldl (:@:) <$> vexpr0 <*> endBy1 expr0 spaces
    <?> "app"
...
inst =
  Inst <$ reserved "inst" <* parens spaces
    <?> "inst"

perform :: MonadicParsing m => m Term
perform =
  Perform <$ reserved "perform" <*> var <*> expr
    <?> "perform"

withH :: MonadicParsing m => m Term
withH =
  WithH <$ reserved "with" <*> vexpr0 <* reserved "handle" <*> expr
    <?> "withH"

handler :: MonadicParsing m => m Term
handler =
  Handler <$> (reserved "handler" *> var)
...
fun =
  Fun <$ reserved "fun" <*> identifier <* arrow <*> expr
    <?> "fun"

int :: MonadicParsing m => m Term
int =
  Int . fromIntegral <$> (spaces *> decimal <* spaces)
    <?> "int"
...
parseE e s = parseString (spaces *> e <* eof) initDelta s
  where
    initDelta = Directed (S.pack s) 0 0 0 0
